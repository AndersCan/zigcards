...
2. **Urdu decks** (section `urdu`): teach everyday Urdu phrases (see §2b-urdu).
   Concept-only: no code, correctness by careful authoring + review.
...
`"prerequisites"` for the memory/foundations decks, or `"urdu"` for the
language decks. Zig decks omit `language` (defaults to `"zig"`). The `Card`
type is defined in `src/types.ts`; `npm run typecheck` enforces the schema.
...
1. **No invented code.** Every snippet is copied from the exercise or healed
   files (ziglings). You may _trim_ a
...
2. **Exact outputs.** `output`-type answers quote the answer key exactly —
   the ziglings elrond key — including

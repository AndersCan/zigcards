...
for (const viewport of VIEWPORTS) {
  test(`grade buttons visible on every card (${viewport.width}x${viewport.height})`, async () => {
    ...
    const deckCount = window.ZigCards.decks.length;
    let reviewed = 0;

    for (let d = 0; d < deckCount; d++) {
      const deckCards = window.ZigCards.decks[d].cards.length;
      ...
      for (let c = 0; c < deckCards; c++) {
        ...
        expect(document.querySelector("#btn-show"), `${where}: Show answer visible`).not.toBeNull();
        expect(
        ...
        for (const id of ["#btn-known", "#btn-unknown"]) {
          ...
          expect(box.y + box.height, `${where}: ${id} bottom inside viewport`).toBeLessThanOrEqual(
            height,
          );
        }
